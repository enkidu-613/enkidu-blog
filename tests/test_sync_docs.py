...
class SyncFilePublishedDateTests(unittest.TestCase):
    """Ensure source mtimes become the displayed publication date."""
    ...
    def test_replaces_published_with_source_mtime_and_removes_updated(self):
    ...
    def test_replaces_published_and_skips_unchanged_second_run(self):
        with tempfile.TemporaryDirectory() as temporary_directory:
            directory = Path(temporary_directory)
            source_path = self.write_source(directory)
            destination_path = directory / "destination.md"
