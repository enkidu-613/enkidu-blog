...
| 标准 | 问题 | 答案在 |
|------|------|--------|
| 思想是什么 | 记忆不是模型内部保存，而是应用在下一次请求时重新传入历史 | 二、核心原理：聊天记录本 `chat_history` |
| 干什么 | 让 AI 能记住前文，回答上下文相关的问题 | 一、为什么需要"记忆"？ |
| 为什么这么干 | 只发送 `req.message` 会让模型忘记前文；全局内存列表适合最小演示，但不能替代按用户和会话隔离的数据库存储 | 六、局限性与改进方向 |
| 怎么干 | 能从 `chat_history.append(...)`、`messages=chat_history` 和两个历史路由找出这条记忆链路 | 四、多轮对话实现 / 五、代码详解 |
