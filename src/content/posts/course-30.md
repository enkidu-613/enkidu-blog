...
| 标准 | 问题 | 答案在 |
|------|------|--------|
| 思想是什么 | Dify 是把模型、知识库、Prompt 和流程节点配置在画布上的平台，把已写过的 RAG 变成可观察、可配置的应用 | 一句话理解 |
| 干什么 | 解决从手写代码到可视化编排的跨越——不替代理解 RAG，而是让你在画布上搭出 Agentic RAG 并发布为 API | 第一关"前置对照：固定 RAG" |
| 为什么这么干 | 纯代码流程改参数要改代码重启；Dify 画布允许非代码人员调试检索参数、工具配置和 Prompt，同时发布为 API 供后端调用 | 第三关"用 Agent 节点复现工具循环" |
| 怎么干 | 创建检索 Workflow（开始 → 知识检索 → 输出）→ 发布为工具 → 创建主 Agent Workflow（开始 → Agent → 输出）→ 从 FastAPI 调用 `/workflows/run` | 第二关、第三关、第四关 |
