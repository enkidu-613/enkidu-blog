...
| 标准 | 问题 | 答案在 |
|------|------|--------|
| 思想是什么 | 上下文窗口是有限座位，必须给资料、问题和回答分配预算 | 一、什么是上下文窗口 |
| 干什么 | 防止输入超限、回答失控和资料挤爆模型 | 三、三层防御策略 |
| 为什么这么干 | 不管理 Token 会导致报错、截断、变慢或幻觉 | 四、为什么不能统一成一个数字 / 五、防幻觉策略 |
| 怎么干 | 能解释三层防御，并知道资料片段超限时为什么用 `break` | 三、三层防御策略 / 八、错题记录 |

---

## 延伸：从"窗口管理"到"上下文工程"

本章前面讲的是**防御**——别让上下文撑爆。2025 年之后，行业给这套事情起了个更上层的名字：**上下文工程（Context Engineering）**。你认得它，聊天和面试都能加分。

### 一句话区分

```text
上下文窗口管理（本章主体）：被动防御 —— "别超限、别撑爆、别幻觉"
上下文工程（本节）：       主动设计 —— "在有限的窗口里，放最有价值的东西"
```

> 类比：窗口管理是"别把行李箱塞爆"（重量控制）；上下文工程是"这趟旅行该带什么"（打包策略）。前者是约束，后者是设计。

### 上下文工程的四个操作

业界把对上下文的操作归纳成四类，每一类你其实都学过：

| 操作 | 干什么 | 你已经学过的对应 |
| --- | --- | --- |
| **Write（写入）** | 把信息存起来，需要时放进上下文 | 27 章 `session_id` 对话记忆、长期记忆存储 |
| **Select（选取）** | 从外部取相关的回来 | 09-13 章 RAG 检索、向量库取回 |
| **Compress（压缩）** | 精简已有内容，省空间 | **本章的三层防御**（截断、限制片段数） |
| **Isolate（隔离）** | 把上下文拆开，各管各的 | 31 章 Subagent（每个子 Agent 独立上下文） |

**这就是上下文工程的价值**：你之前学的 RAG、记忆、截断、多 Agent，**不是零散技巧，而是同一个框架的四个动作**。
...
```text
  ...
  Write      Select    Compress    Isolate
   写入       选取       压缩       隔离
    │          │          │          │
  记忆存储   RAG检索   截断/摘要   多Agent
  对话历史   工具结果  去重/合并   独立上下文
```

**真实的 Agent 系统永远是四个动作的组合**：

```text
用户提问
  -> Select：检索相关文档 + 取回相关记忆
  -> Compress：压缩到预算内
  -> （复杂任务）Isolate：拆给多个子 Agent 各自处理
  -> 生成回答
  -> Write：把有价值的结论存进记忆
```
...
1. **串起你的知识**：你学的 RAG、Memory、截断、多 Agent 不再是一堆孤立章节，而是一个体系的四个面
2. **行业共识**：2025 年后，"Prompt Engineering"这个词在衰减，"Context Engineering"在上升——因为大家发现**决定 Agent 好坏的不是措辞，是喂给模型什么信息**
3. **面试加分**：能说出"上下文工程四操作"并对照自己的项目讲，比只会说"我做过 RAG"强一档

### 本章边界

本节只要求你**认得这个框架**。你当前项目的三层防御已经够用，不需要现在就重构。等你做多 Agent 系统（31 章之后）或需要跨会话记忆时，再回来看这张四操作图，会发现它们正好各就各位。
