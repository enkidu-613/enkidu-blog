...
- **本章要会**: 把双存储脚本封装成 `POST /rag/documents` 和 `POST /rag/search` 两个 API 接口，理解 `PersistentClient` 持久化，能在 `main.py` 注册新 router 并用 curl 测试。
- **本章暂不要求**: 删除/修改文档接口、用户权限、批量入库、分页查询、生产级错误处理和异步任务队列。这些属于生产增强，不阻塞当前"脚本变 API"的最小链路。
...
| 标准 | 问题 | 答案在 |
|------|------|--------|
| 思想是什么 | 先把向量检索脚本变成能被外部调用的 API | 1. 为什么需要 API 化？ |
| 干什么 | 提供文档存入和语义检索两个最小接口 | 2. 接口设计 |
| 为什么这么干 | API 化后前端、测试脚本、其他服务才能复用 RAG 能力 | 1. 为什么需要 API 化？ |
| 怎么干 | 能启动服务，用 curl 调 `/rag/documents` 和 `/rag/search`，并知道数据会持久化到哪里 | 6. 完整代码 / 7. 测试接口 |
