...
draft: false
---
> 本章目标：复用第 24 章的 `expected_document` / `expected_fact` 思路，区分“没检索到”“检索到了但没答对”“检索和答案都暂时通过”三类结果。
>
...
RAG 回答失败，不等于模型一定不好；先分别看有没有拿到预期资料，以及答案文本是否包含关键事实。
...
这两个检查是最小回归信号，不足以证明模型“因为该来源才得出答案”，也不能证明答案整体绝对正确。当前契约没有检查引用 ID；如果要验证来源因果关系，下一步应把最终答案携带的 `citation_document_ids` 也纳入契约。
