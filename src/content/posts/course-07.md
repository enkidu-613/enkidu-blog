...
- **本章要会**: 说清四层（表现层/业务层/模型层/数据层）的职责，能用 `APIRouter` 拆分路由并在 `main.py` 注册，能解释分层后导入关系为什么是单向的。
- **本章暂不要求**: `services/` 层、`schemas/` 层、异步路由、中间件解耦和领域驱动设计。这些属于更大项目的架构手段，当前掌握四层分包即可。
...
| 标准 | 问题 | 答案在 |
|------|------|--------|
| 思想是什么 | 关注点分离，每个文件只承担一种职责 | 一、为什么要分层 |
| 干什么 | 让项目变大后仍然能定位、修改、测试和复用代码 | 一、为什么要分层 |
| 为什么这么干 | 单文件会导致阅读困难、冲突多、循环依赖和复用差 | 1.1 问题场景：单体文件的困境 |
| 怎么干 | 能创建 `APIRouter`、在 `main.py` 注册，并说清 `database.py`、`models.py`、`routers.py` 的职责 | 三、各层代码解析 / 四、APIRouter 深度解析 |
