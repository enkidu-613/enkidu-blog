...
ReAct 只是 Agent 的**一种**思路。就像做菜有"边做边尝"和"按菜谱一次做完"两种流派，Agent 也有三种经典范式，你至少要认得名字：

| 范式 | 一句话 | 生活类比 |
| --- | --- | --- |
| **ReAct** | 想一步、做一步、看结果再想 | 摸着石头过河 |
| **Plan-and-Solve** | 先把整件事规划成步骤，再一步步执行 | 先画图纸，再施工 |
| **Reflection** | 先做一版，再回头挑毛病、改一版 | 写完作业检查一遍 |

下面两个是你还没见过的。

### Plan-and-Solve：先规划，再执行

ReAct 是"走一步看一步"，**Plan-and-Solve 反过来：先让模型把整个任务拆成计划，再按计划执行**。

```text
用户目标："帮我查一下订单 123 的状态并给出处理建议"
  ↓ 第一步：Planner 输出计划
plan = [
  "1. 调用 query_order(123) 查订单",
  "2. 调用 search_policy('退款') 查规则",
  "3. 综合两者给出建议"
]
  ↓ 第二步：Executor 按计划执行（逐步调用工具）
执行 1 → 得到订单状态
执行 2 → 得到退款规则
执行 3 → 生成最终答案
```
...
| --- | --- | --- |
| 计划在哪产生 | 边执行边产生（每步都重新想） | **执行前一次性产生** |
| 中途会改计划吗 | 会（每步看结果决定） | 通常不改（除非失败重规划） |
| 步数 | 不可预测 | 计划出来就知道大概几步 |
| 适合 | 探索性任务（不知道要查几次） | **步骤明确的任务** |

**什么时候用它**：任务能拆成清晰步骤、你希望成本可控（步数可预测）。  
**什么时候别用**：任务本身需要探索（比如"帮我查查为什么服务挂了"，你事先不知道要查几个地方）——那种用 ReAct。

> 类比：搬家。Plan-and-Solve 是"列好清单：先装书、再装衣服、最后装锅"；ReAct 是"打开柜子看一眼，决定这个箱子装什么"。柜子里有什么你事先不知道时，只能边看边装。

### Reflection：做完再复盘一版

**Reflection 的思路：让 Agent 先产出结果，再让它（或另一个 Agent）审视这个结果，挑出问题，然后改一版。**
...
draft = "订单已发货，建议等待"
  ↓ 第二轮：Reflector 审视
feedback = "没核实订单是否真发货；没引用退款规则"
  ↓ 第三轮：Generator 根据反馈修改
final = "订单 123 已于 8/1 发货（来源：query_order），根据退款规则第 3 条..."
  ↓ （可选）重复直到满意或达到上限
```
...
| 角色 | 干什么 |
| --- | --- |
| **Generator（生成者）** | 负责产出答案 |
| **Reflector（审视者）** | 负责挑毛病——可以是同一个模型换个 prompt，也可以是另一个模型 |

**为什么有效**：人写东西第一版往往有遗漏，"检查一遍"能抓到明显问题。模型也一样——**让它自己审自己，比让它一次写对容易**。

**代价**：每一次反思都是一次额外的 LLM 调用，token 和时间都翻倍。**别什么都反思**。
...
| --- | --- |
| 不知道要调几次工具（探索性） | **ReAct** |
| 步骤能提前列清楚（流程性） | **Plan-and-Solve** |
| 一次写不好，需要打磨质量（写作/代码） | **Reflection** |
| 又要探索又要质量 | ReAct + Reflection（先探索，最后复盘一次） |

**你当前项目（25-29 章的 Agent）用的是 ReAct**——`langchain_agent.py` 里模型自己决定调不调工具，就是边走边看。等你要做"步骤固定"的批处理任务时，再考虑 Plan-and-Solve；要提升答案质量时，加一层 Reflection。
