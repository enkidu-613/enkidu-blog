...
| 版本 | 含义 |
|:----:|------|
| `$2a$` | 原始版本，存在一个 2011 年发现的溢出 bug（密码超 55 字符时被截断） |
| `$2b$` | **修复版**（2014），修正了 `$2a$` 的截断 bug，当前默认 |
| `$2y$` | PHP 早期对 `$2b$` 的别名，实际等价 |
| `$2x$` | PHP 的"bug-compatible"模式，刻意保留 `$2a$` 的 bug |

你的项目用的是 `$2b$`（`bcrypt.gensalt()` 默认），这是正确的现代选择。
