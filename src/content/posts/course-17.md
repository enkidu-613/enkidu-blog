...
> 忘了 JWT 怎么传？`Ctrl+F` 搜 "Bearer"，先看请求头和验证流程，再对照模板。类比只辅助理解，不代替签名、过期和权限检查。
...
- **本章要会**：看懂 JWT 三段结构、会用 bcrypt 哈希密码、用 `Depends(get_current_user)` 保护路由、用黑名单实现登出、区分 401 和 403。
- **本章暂不要求**：实现长短 Token 双卡制、OAuth/OIDC 协议、生产级 Redis 黑名单。
...
### 🔬 bcrypt hash 内部解剖（盐、成本因子、版本号）

生成出来的 hash 不是一串乱码，它有严格的 60 字符结构：

```
$2b$10$N9qo8uLOickgx2ZMRZoMyeIjZIMs3xXcJ3xMBXmq...
 ─┬─ ─┬─ ───────────┬────────────── ──────────┬──────────
  │   │       盐 (22 字符)              哈希值 (31 字符)
...
- **来源**：`bcrypt.gensalt()` 调用操作系统的加密安全随机数生成器（Linux 读 `/dev/urandom`，macOS 用 Security.framework）
- **作用**：即使两个用户密码相同，因为盐不同，最终 hash 完全不同
- **不用单独存**：盐直接嵌在 hash 字符串前 22 字符里，验证时 checkpw 自动提取

#### 🔢 成本因子（Cost Factor）—— `$10$`

- `rounds=10` 意思是 **2^10 = 1024 次**迭代哈希
- `rounds=12` → 2^12 = **4096 次**，是 rounds=10 的 **4 倍**
- `rounds=14` → 2^14 = **16384 次**，是 rounds=10 的 **16 倍**

> ⚠️ **为什么不是越大越好？** 成本因子每 +1，哈希时间翻倍。rounds=14 会让每次登录等 ~1.6 秒，用户体验崩了。业界建议让 hash 耗时控制在 **100ms~300ms**，根据服务器性能调。

| rounds | 迭代次数 | 大致耗时 | 适合场景 |
|:------:|:--------:|:--------:|----------|
| 8 | 256 | ~6ms | ❌ 太弱 |
| **10** | **1024** | **~100ms** | ✅ 学习/一般应用 |
| 12 | 4096 | ~400ms | 生产环境推荐 |
| 14 | 16384 | ~1.6s | ⚠️ 高安全但慢 |

#### 📜 版本号 —— `$2b$`

这里先认识 `$2b$` 是 bcrypt 哈希字符串中的算法版本标记，`bcrypt.gensalt()` 默认使用这个前缀。不同历史前缀的兼容细节不属于本章登录流程，不要求背诵。

不要把前缀与密码长度限制混为一谈：长度按字节而非汉字个数计算；当前官方实现对超过 72 字节的输入报错，旧版本可能截断，需按项目安装版本确认。不要自行截断密码或随意改写已存哈希的前缀。依据：[pyca/bcrypt 官方说明](https://github.com/pyca/bcrypt#maximum-password-length)。
...
### 一句话
本章使用签名 JWT：`Header（头部）.Payload（载荷）.Signature（签名）`。Payload 是保存用户 ID、过期时间等声明的数据；签名用于验证内容是否被篡改，不负责保密。
...
**安全边界**：本章签名 JWT 的头部和载荷使用 Base64url 编码，并未加密。拿到令牌的人可以读出载荷，不需要签名密钥；但“读出来”不等于“验证通过”。不要把真实访问令牌粘贴到第三方网站，也不要把密码等秘密写进载荷。依据：[RFC 7519](https://datatracker.ietf.org/doc/html/rfc7519)。

Base64url 是把字节表示为适合 URL 的字符形式，属于编码而非加密。本章由 `jwt.encode()` / `jwt.decode()` 处理，不要求手写编码算法；第 33 章本地图片输入再讲编码的具体调用。
...
> ⚠️ **为什么登出用 `Depends(security)` 而不用 `Depends(get_current_user)`？** `get_current_user` 会先查黑名单再验签，对于已过期或需挂失的 Token 直接抛 401，根本走不到黑名单写入这一步。登出时只需要**提取 Token 字符串**（HTTPBearer 就能做到），然后无条件尝试加入黑名单——包括已过期的 Token（`verify_exp=False`）。
...
| Payload 放密码或不必要的个人信息 | Base64url 可解码，签名不提供保密性 |
| 不设 Token 过期时间 | 一旦签发，永不过期 |
| 登录失败提示"密码错误" | 暴露账号存在性，方便撞库 |
| 只用 JWT 无黑名单实现登出 | 退出后 Token 依然可用 |
| .env 提交到 Git | 密钥永久留在版本历史里 |
| 配置文件权限过宽或被公开 | 未授权者可能读取密钥，不能只靠文件名保护 |
| 黑名单存完整 Token | 增加令牌泄露面；本项目存令牌哈希用于匹配 |
...
├── JWT 令牌
│   ├── Header: 算法声明
│   ├── Payload: 用户ID/角色/过期时间（编码可读，不放密码）
...
| 标准 | 问题 | 答案在 |
|------|------|--------|
| 思想是什么 | JWT 是无状态身份卡 + 服务器端密码哈希 + 黑名单补丁，既享受无状态又维护可控撤销。 | 第一关「认证 vs 授权」、第三关「JWT 三段式」、第八关「两个致命缺陷与补丁」 |
| 干什么 | 解决"谁在访问"（认证）和"能访问什么"（授权），不用每次请求都查用户表。 | 第一关「认证 vs 授权」、第三关「JWT」 |
| 为什么这么干 | 无状态 Token 免去每次查库，哈希防密码泄露，黑名单补 Token 不可撤销的缺陷。 | 第二关「密码哈希」、第八关「黑名单」 |
| 怎么干 | 复制 `app/routers/auth.py` 的 `create_token` + `get_current_user` + `require_role`，按第十关 curl 模板跑通注册→登录→/me→登出→401 链路。 | 第十关「实战验证」curl 命令
