...
| 标准 | 问题 | 答案在 |
|------|------|--------|
| 思想是什么 | LangChain 用统一对象和可调用链组织 RAG 流程，不会凭空替你产生知识库答案。 | 一、为什么需要 LangChain |
| 干什么 | 减少手写步骤之间的参数对接和重复胶水代码，让流程更容易组合与替换。 | 一、手搓版 vs 框架版 |
| 为什么这么干 | 只有遵守 Runnable 输入输出约定的对象才能组成 LCEL；HTTP 路由、鉴权和 SSE 响应仍是应用层职责。 | 二、LCEL 管道符、八、手搓版 vs LangChain 版完整对照表 |
| 怎么干 | 从 `app/routers/langchain_rag.py` 找到“检索、格式化 Prompt、调用模型、流式输出”分别落在哪个对象或函数上，抄十、速查表模板。 | 十、速查表
