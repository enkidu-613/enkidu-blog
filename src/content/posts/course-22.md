...
- **本章要会**：区分 Prompt、Schema、服务端权限分别能防什么；给 AI 接口加最小安全壳（输入检查 + Prompt 隔离 + 输出校验）；知道 RAG 必须先按权限过滤再检索；写出最小安全检查函数。
- **本章暂不要求**：内容审核 API 集成、对抗性测试、法律合规细则、联邦学习或差分隐私。
...
| 标准 | 问题 | 答案在 |
|------|------|--------|
| 思想是什么 | AI 安全不是只写 Prompt，而是给模型输入、输出和工具执行加后端边界。 | 第一关「AI 安全到底在防什么」、第二关「Prompt 不是安全边界」 |
| 干什么 | 防 Prompt Injection、数据泄露、越权工具调用、RAG 上下文泄露和危险内容输出。 | 第一关「准确术语」、第四关「本项目里的风险点」 |
| 为什么这么干 | 只靠 Prompt 不够，因为模型不是确定性权限系统；只靠 Schema 不够，因为 Schema 只校验结构，不理解意图。 | 第二关「为什么不是安全边界」、第三关「Prompt、Schema、权限分别能防什么」 |
| 怎么干 | 加最小安全壳：输入校验 → Prompt 隔离 → Schema 校验 → 服务端鉴权 → 日志记录；参考 `app/ai_safety.py` 模板。 | 第五关「最小安全壳模板」、第十关「本项目最小实战任务」
