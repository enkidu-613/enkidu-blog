...
| 标准 | 问题 | 答案在 |
|------|------|--------|
| 思想是什么 | 双存储不是重复造库，而是把"语义找候选"和"可靠保存、回查业务数据"拆给合适的系统 | 🤔 为什么需要双存储？ |
| 干什么 | 让 SQLite 负责完整文档和关系查询，ChromaDB 负责语义检索 | 🤔 为什么需要双存储？ |
| 为什么这么干 | SQLite 不擅长语义近邻检索，ChromaDB 也不应承担完整业务文档、事务和关系查询 | 🤔 为什么需要双存储？ |
| 怎么干 | 能沿着 `Document`、`DocumentChunk`、`embedding_id`、`metadatas` 和 `collection.query()` 说出写入与回查顺序 | 💻 完整代码流程 |
