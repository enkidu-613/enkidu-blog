...
2. [循环控制](#二循环控制)
3. [字符串操作](#三字符串操作)
4. [元组解包（Tuple Unpacking）](#四元组解包tuple-unpacking)
5. [集合（Set）](#五集合set)
6. [列表进阶操作](#六列表进阶操作)
7. [函数进阶](#七函数进阶)
8. [异常处理](#八异常处理)
...
本章把第一章的 Python 基础语法连接成可复用的程序结构：先用条件和循环控制流程，再用字符串、集合、列表和函数处理数据，最后用异常、文件、模块和继承组织真实代码。

### 本章学到哪里，不学到哪里

**本章需要掌握：**

- 比较、逻辑、成员运算符，以及 `and`、`or` 的短路求值。
- `while`、`for`、`break`、`continue` 的执行时序。
- 字符串/列表/元组切片、常用字符串方法、元组解包、集合和列表推导式。
- 函数的默认参数、可变参数、返回值和作用域基础。
- `try-except-finally`、`with open(...)`、模块导入和基础继承。

**本章暂不展开：**

- 装饰器、生成器、异步函数、类型标注和数据类；这些内容在后续章节单独学习。
- 多继承、抽象基类、方法解析顺序和复杂设计模式；本章只要求能读懂基础父子类关系。
- `numpy`、`pandas` 的数据分析 API；这里只识别它们的导入形式，不要求现在安装或掌握。

### 本章准确术语

| 术语 | 它在 Python 中是什么 | 本章要看懂的关键点 |
|------|----------------------|--------------------|
| 比较运算符 | 产生布尔结果的运算符 | `1 < x < 10` 是 Python 合法的链式比较 |
| 短路求值 | 逻辑表达式的执行规则 | `and` 或 `or` 可能不会执行右侧表达式 |
| 切片（slice） | 从序列中按 `start:end:step` 取子序列 | `end` 不包含在结果中 |
| 可变对象 | 内容可以原地修改的对象 | 列表可变，元组和字符串不可变 |
| 异常（exception） | 程序运行期间报告错误情况的对象 | 用 `try-except` 处理预期错误 |
| 上下文管理器 | 管理资源进入和退出的协议 | `with open(...)` 结束后自动关闭文件 |
| 模块（module） | 一个可被导入的 Python 文件 | `import` 负责加载和命名空间组织 |
| 继承（inheritance） | 子类复用或重写父类行为的机制 | `super()` 可以调用父类实现 |

### 本章最小代码地图

下面这段不是要替代后面的分节练习，而是先把本章的调用形状串起来：

```python
def parse_score(text):
    """把输入文本转换为 0 到 100 之间的整数。"""
    try:
...
**调用链：** `open` 打开文件 → `with` 管理文件生命周期 → `parse_score` 清理并转换字符串 → `if` 判断范围和结果 → `except` 处理可预期错误。

### 本章首次出现的库说明

本章主要使用 Python 标准库和语言内置能力，不需要额外安装依赖。

| 库/名称 | 类型 | 当前用途 | 常见业务用途 | 本章边界 |
|---------|------|----------|--------------|----------|
| `math` | Python 标准库 | 示例中的数学函数，如 `sqrt` | 几何、统计和数值计算 | 只需会导入和调用，不学数值算法 |
| `random` | Python 标准库 | 示例中的随机数能力 | 测试数据、抽样和简单模拟 | 不用于安全令牌或密码 |
| `time` | Python 标准库 | 示例中的时间/暂停函数 | 重试间隔、简单计时 | 不展开异步调度 |
| `numpy` | Poetry 管理的第三方库示例 | 展示 `import ... as ...` 形式 | 数组和数值计算 | 本章只识别，不要求安装 |
| `pandas` | Poetry 管理的第三方库示例 | 展示数据分析库的别名导入 | 表格数据清洗和分析 | 本章只识别，不要求安装 |

`math`、`random` 和 `time` 随 Python 提供；`numpy`、`pandas` 不属于 Python 内置库。不要因为示例出现了第三方库，就把它们误认为本章的学习重点。

### 常见坑预警

1. Python 支持 `1 < x < 10`，不要把它误写成 JavaScript 风格的 `1 < x && x < 10`。
2. `start` 包含、`end` 不包含；切片不会修改原字符串或原元组。
3. `continue` 只跳过当前轮，`break` 才会结束整个循环。
4. 可变默认参数如 `items=[]` 会在多次调用之间共享，函数默认使用 `None` 更安全。
5. `except Exception` 会兜底很多错误，业务代码优先捕获具体异常。

### 开始前检查点

学习完本章后，你应该能回答并实际验证：

- 核心思想是什么：如何把基础语法组合成可执行的数据处理流程？
- 它解决什么问题：循环、异常、文件、模块和继承分别减少了什么重复或风险？
- 为什么不用替代方案：什么时候用 `break`，什么时候用条件控制；为什么用 `with` 而不是手动关闭文件？
- 在本项目里怎么识别：看到 `try`、`with open`、`from ... import ...` 和 `class Child(Parent)` 时，能说出输入、调用和结果？
...
**注意：** Python 支持像数学一样的链式比较；它不是把两个比较结果直接当作普通数字相连，而是按顺序判断整个条件：
```python
# 判断 x 是否在 1 和 10 之间
...
元组解包是 Python 的一个强大特性：将**元组（或列表）的每个元素**直接赋值给**多个变量**。
...
```
第1次循环: text, expected = ("我喜欢吃苹果", "都关于苹果")
...
| 场景 | 示例 |
|------|------|
| 遍历字典的键值对 | `for k, v in dict.items()` |
| 遍历枚举 | `for i, item in enumerate(list)` |
| 两个列表配对遍历 | `for a, b in zip(list1, list2)` |
| 函数返回多个值 | `name, age = get_person()` |
| 交换变量 | `a, b = b, a` |

```python
# 遍历字典（还记得 01_Python基础 中学到的 .items() 吗？）
person = {"name": "Alice", "age": 18}
...
完成练习后，不要只背语法，至少完成下面四个验证：

1. 写一个循环，从列表中找到第一个满足条件的元素，并说明 `break` 在哪一轮结束循环。
2. 写一个函数，把用户输入转换成整数；输入不是数字时捕获 `ValueError`，不要使用裸 `except`。
3. 用 `with open(...)` 写入再读取一个文本文件，并说明文件何时关闭。
4. 写一个父类和一个子类，让子类重写一个方法，再用 `super()` 复用父类初始化逻辑。

如果这四项都能独立完成并解释输入、调用和输出，本章才算真正掌握。
...
| 标准 | 问题 | 答案在 |
|------|------|--------|
| 思想是什么 | 把基础语法组合成可复用的数据处理流程：运算符控制数据流向，异常、文件和模块管理副作用和依赖。 | 开始前检查点 |
| 干什么 | 解决"程序怎么处理真实数据而不崩溃"——用循环遍历、用异常容错、用文件持久化、用模块组织代码、用继承复用行为。 | 开始前检查点 |
| 为什么这么干 | 每种特性解决一类重复：`with open` 自动关闭文件，`try` 避免崩溃，`break`/`continue` 精确控制循环，`super()` 复用父类逻辑。 | 常见坑预警 |
| 怎么干 | 能写出链式比较、切片、`try-except-finally`、`with open`、`from ... import` 和 `class Child(Parent)` 的最小模板。 | 本章最小代码地图 · 总结速查表
