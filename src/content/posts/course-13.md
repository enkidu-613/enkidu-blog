...
| 标准 | 问题 | 答案在 |
|------|------|--------|
| 思想是什么 | RAG 不是"搜到资料"就结束，而是把资料和回答规则一起交给模型 | 🎯 一句话理解 / 📖 生活类比 |
| 干什么 | 让回答尽量依据私有资料，并能给出来源，而不是只依赖模型原有知识 | 💻 代码总览 |
| 为什么这么干 | 普通聊天接口没有自动得到本次检索结果，也没有"找不到资料就拒答"的上下文边界 | 🔍 零件②：build_system_prompt() |
| 怎么干 | 能按顺序说出 `rag_chat`、`_semantic_search`、`build_system_prompt`、`generate_rag_stream` 各自负责什么 | 🔍 零件 ①-④ |
