...
可以辅助理解为“语义表示”，但不是唯一指纹：向量不保证文本身份唯一，也不能当正文哈希用于精确去重。下面数字是示意，维度由具体模型决定。
...
| 标准 | 问题 | 答案在 |
|------|------|--------|
| 思想是什么 | 向量库按语义距离找内容，而不是按字面字符串匹配 | 一、为什么需要向量数据库？ |
| 干什么 | 让用户换一种说法时仍能命中相关文档 | 1.1 传统搜索的"盲区" |
| 为什么这么干 | SQL `LIKE` 是字面匹配，不理解语义；ChromaDB 擅长语义召回，但不能替代保存业务完整数据的关系型数据库 | 三、向量数据库 vs 关系型数据库 / 五、生产级设计：双存储架构 |
| 怎么干 | 能指出 `PersistentClient`、`get_or_create_collection()`、`add()`、`query()` 在最小原型中的位置 | 四、ChromaDB 实战 / 六、RAG 最小原型 |
