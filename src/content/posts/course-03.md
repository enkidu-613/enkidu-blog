...
- **本章要会**：用列表推导式替代简单 for 循环生成列表；用生成器表达式处理大数据集；用 `yield` 写生成器函数管理资源和流式数据；用装饰器给函数添加前置/后置逻辑。
- **本章暂不要求**：`async def` / `await` 异步编程、`asyncio` 事件循环、类型注解的深度使用（`TypedDict`、`Generic`）、元类和 `__dunder__` 方法定制。
