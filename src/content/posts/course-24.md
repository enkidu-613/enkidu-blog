...
- **本章要会**：区分 retrieval / context / answer 三层评估、用当前项目的 `/search` 和 `/chat` 接口做手工评估、准备 5-10 条 golden questions、用评估表记录 `top1_hit` / `top3_hit` / `groundedness`、调参时一次只改一个变量。
- **本章暂不要求**：RAGAS / LangSmith 自动化评估（后面章节专讲）、生产级评估数据集、A/B 测试框架、回答长度与延迟等非功能指标。
...
| 标准 | 问题 | 答案在 |
|------|------|--------|
| 思想是什么 | 给 RAG 建一套体检表，检查检索是否命中、上下文是否干净、答案是否基于资料 | 一句话理解 |
| 干什么 | 解决"看起来能答，但不知道是否真的可靠"的问题 | 第一关"为什么不能只看模型答得像不像" |
| 为什么这么干 | 肉眼看最终回答不够，因为模型很会把错误说得像真的；一上来就用自动指标又不知道每分为什么扣 | 第五关"为什么先手工评估，再自动评估" |
| 怎么干 | 用 `/langchain-rag/search` 和 `/langchain-rag/chat` 配合评估表记录 top1_hit / top3_hit / groundedness | 第三关、第四关 |
