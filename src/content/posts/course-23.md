---
title: "23. RAG Chunking 策略：资料怎么切，检索才不傻"
published: 2026-01-23
description: "本章参考的是官方/一手文档，并结合你当前项目实现改写成学习版："
tags: ["AI 应用工程", "学习笔记"]
category: "AI 应用工程"
draft: false
---
...
- **本章要会**：判断文档该用什么切法、调 `chunk_size` / `chunk_overlap` / `separators`、用手搓版和 LangChain 版分别切一段文本、用 metadata 关联 chunk 与来源、通过 chunks_count 和检索结果判断切分质量。
- **本章暂不要求**：token 级精确切分、语义切分（如基于 embedding 的合并）、生产级分块参数调优、RAG 完整评估流程（下一章专讲）。
...
| 标准 | 问题 | 答案在 |
|------|------|--------|
| 思想是什么 | 把长文档切成适合向量检索的小语义单位 | 一句话理解 |
| 干什么 | 解决整篇文档太大导致检索噪声多、切太碎导致答案上下文断掉的问题 | 第一关"Chunk 到底是什么" |
| 为什么这么干 | 固定长度硬切不懂段落和句子边界；复杂语义切分当前阶段过早引入 | 第四关"为什么不用手搓固定长度切片" |
| 怎么干 | 抄 `RecursiveCharacterTextSplitter(chunk_size=500, chunk_overlap=50, separators=[...])` | 本章最小模板 |
